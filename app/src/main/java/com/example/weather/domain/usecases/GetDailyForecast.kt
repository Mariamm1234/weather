...
class GetDailyForecast @Inject constructor(
    ...
    operator fun invoke(
        ...
        lang: String,
        ctx: Context
    ): Flow<Resource<forecastDaily>> =
        flow {
            try {
                emit(Resource.Loading())
                Log.i("weather", repo.getForecastDaily(lon,lat,lang,ctx).value.toString())
                var weather=repo.getForecastDaily(lon,lat,lang,ctx).value
        ...
        }.flowOn(Dispatchers.IO) as Flow<Resource<forecastDaily>>
}